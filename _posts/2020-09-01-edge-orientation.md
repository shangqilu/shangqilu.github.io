...
The analysis in \[BB20\] is non-trivial, which shows that the algorithm is competitive to the off-line strategy in Lemma 1. Here I only give some basic ideas about the proof. The first interesting lemma is that after $k$ max-flips, the max out-degree increased by at most one. Hence, a max-flip is performed on a vertex with out-degree $c \cdot \alpha_h$ for some constant $c$, after the $k$ max-flips, $G^+$ is still an $O(\alpha_h)$-orientation. 
